...
def calc_reward(env, env_state, action, next_env_state, prev_opp_state, opp_state, done):
    ...
    # Bonus for dealing damage while the opponent is acting
    damage_done = (env_state[65] - next_env_state[65])
    opponent_acting = env_state[129]
    if damage_done > 0 and (opponent_acting < 0.9 and opponent_acting > 0.1):
        reward += 100
...
def main():
    ...
    for episode in range(n_episodes):
        ...
        agent.save('./aggressive.pt', epsilon, rewards, wins, damage_done, damage_taken)
