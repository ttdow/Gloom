...
def main():
    ...
    opponent_file_list = ["EggTart"]
    #opponent_file_list = ["BlackMamba"]
    #opponent_file_list = ["Thunder2021"]
    ...
    for opponent_file in opponent_file_list:
        ...
        state = env.reset(p2=opponent_file)
        ...
        okiAgent = Agent(state.shape[0], len(action_vecs), learning_rate, gamma, tau, alpha, n_layers, n_episodes)

        neutral_memory = ReplayMemory(100000)
        oki_memory = ReplayMemory(100000)

        agent.load(neutral_file)
        okiAgent.load(oki_file)
